...
sum_2016 <- read_csv('/Users/levisolomyak/Desktop/02Phd/tennis_project/stan_results/abs_model/full_summary_2016.csv')
...
compute_serve_speed <- function(data, params, stan) {
  serve_indices <- unlist(stan$is_serve_speed_indices)
...
create_serve_cal_plot <- function(data, title,replication=0) {
  set_color=ifelse(replication==1,'#FFB140',"#2171B5")
  
  ggplot(data %>% dplyr::filter(n_points>200), aes(x = predicted_speed, y = actual_speed)) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray50", alpha = 1) +
    geom_point(aes(size = n_points), color = set_color, alpha = 0.8) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                  width = 2, color = "#2171B5", alpha = 0.8) +
...
p1 <- create_serve_cal_plot(cal_2011, "2011-2015")
...
player_plots <- p_djokovic + p_nadal + p_federer + 
  plot_layout(guides = "collect") & 
  theme(legend.position = "none")

final_plot <- plot_grid(
  player_plots, shared_legend,
  rel_widths = c(3, 0.7),
  nrow = 1
)
ggsave(
