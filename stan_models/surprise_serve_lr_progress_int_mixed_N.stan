data {
  ...
  int n_serve_speed;
  int n_players;
  int n_players_enough;
  array[n_players_enough] int players_enough_indices;
  int n_matches;
  int n_serve_speed_matches;
  array[n_score] int Point_Server_id;
  array[n_serve_speed] int Point_Server_id_serve_speed;
  ...
  array[n_serve_speed] int<lower=0> serve_speed_match_id;
  array[n_score] int<lower=0,upper=1> y;
  vector<lower=-1,upper=1>[n_score] player1_served;
  vector<lower = -1, upper=1>[n_score] sign;
  vector[n_score] progress_1minus2_abs;
  vector[n_serve_speed] progress_server;
  array[n_serve_speed] int<lower=0> is_serve_speed_indices;
  int n_players_with_serve_speed;
  int n_players_with_serve_speed_enough;
  array[n_players_with_serve_speed_enough] int players_with_serve_speed_enough_indices;
  vector[n_score] ServeNumber;
  array[n_serve_speed] real serve_speed;
  array[n_score] int<lower=0> PointServer;
}
...
parameters {
  ...
  array[n_serve_speed_matches] vector[2] beta_serve_speed_match_raw;
...
transformed parameters {
  ...
  array[n_serve_speed_matches] vector[2] beta_serve_speed_match;
  for (i in 1:n_serve_speed_matches) {
    beta_serve_speed_match[i] = beta_serve_speed_match_raw[i]*sigma_serve_speed_match;
  }
  ...
  for (i in 1:n_players_enough) {
    beta_player_int_dev[players_enough_indices[i]] = beta_player_int_raw[i]*sigma_beta_player_int;
  }
  
  sum_base_predictors = alpha + beta_player[Point_Server_id] - beta_player[Point_non_Server_id] + beta_match[match_id].*player1_served + (beta_player_int_dev[Point_Server_id] - beta_player_int_dev[Point_non_Server_id]).*progress_1minus2_abs;
  
  for (i in 1:n_players_with_serve_speed_enough) {
    beta_serve_number_speed_plus_1[players_with_serve_speed_enough_indices[i]] = beta_serve_number_speed_plus_1[players_with_serve_speed_enough_indices[i]] + beta_serve_number_speed_plus_1_player_raw[i]*sigma_beta_serve_number_speed_plus_1_player;
    beta_serve_speed_player_int_dev[players_with_serve_speed_enough_indices[i]] = beta_serve_speed_player_int_raw[i]*sigma_beta_serve_speed_player_int;
  }
  
  sum_base_predictors_serve_speed = alpha_serve_speed_raw + (alpha_serve_speed_int + beta_serve_speed_player_int_dev[Point_Server_id_serve_speed]).*progress_server + beta_serve_speed_player[Point_Server_id_serve_speed] + (beta_serve_number_speed_plus_1[Point_Server_id_serve_speed] - 1) .* (ServeNumber[is_serve_speed_indices]-1.5)  + to_vector(beta_serve_speed_match[serve_speed_match_id, 1]).*(2 - to_vector(PointServer[is_serve_speed_indices]))  + to_vector(beta_serve_speed_match[serve_speed_match_id, 2]).*(to_vector(PointServer[is_serve_speed_indices]) - 1);
  
  for (i in 1:n_players_enough) {
    beta_surprise_player_dev[players_enough_indices[i]] = beta_surprise_player_raw[i]*sigma_beta_surprise_player;
    beta_surprise_int_player_dev[players_enough_indices[i]] = beta_surprise_int_player_raw[i]*sigma_beta_surprise_int_player;
  }
  
  beta_surprise_player1 = beta_surprise_raw + beta_surprise_player_dev[id1] + (beta_surprise_int_raw + beta_surprise_int_player_dev[id1]).*progress_1minus2_abs;
  beta_surprise_player2 = beta_surprise_raw + beta_surprise_player_dev[id2] + (beta_surprise_int_raw + beta_surprise_int_player_dev[id2]).*progress_1minus2_abs;
  
  for (i in 1:n_players_with_serve_speed_enough) {
    beta_serve_speed_surprise_int_player_dev[players_with_serve_speed_enough_indices[i]] = beta_serve_speed_surprise_int_player_raw[i]*sigma_beta_serve_speed_surprise_int_player;
    beta_serve_speed_surprise_player_dev[players_with_serve_speed_enough_indices[i]] = beta_serve_speed_surprise_player_raw[i]*sigma_beta_serve_speed_surprise_player;
  }
...
model {
  ...
  for (i in 1:n_serve_speed_matches) {
    target += std_normal_lpdf(beta_serve_speed_match_raw[i]);
  }
...
generated quantities{
  ...
  for (i in 1:n_players_with_serve_speed_enough) {
    beta_serve_number_speed_player[players_with_serve_speed_enough_indices[i]] = beta_serve_number_speed_player[players_with_serve_speed_enough_indices[i]]  + beta_serve_number_speed_plus_1_player_raw[i]*sigma_beta_serve_number_speed_plus_1_player;
  }
